...
| `urn:iki:finding:{id}` | one **pending review finding** — a `prov:Entity`, **not** an `oa:Annotation`. Source reads it (drift-reconciled like an annotation); Sink is the human's answer: `decision=publish` mints the annotation (needs `urn:cap:annotate`) and `decision=decline` keeps the finding as a record that someone looked and said no. `severity=` is the human's FINAL rating from a closed `one_of`; the model's proposal is never overwritten, and a piped value is the human's free-text note. Since 0.10.0 a decline may also say WHY in one word — `reason=` from a closed `one_of` (`misread` · `restates` · `no-issue` · `wont-fix` · `duplicate`, each word's meaning in the ArgSpec summary), stored as the term `urn:iki:decline-reason:{word}` and read back as `decision.reason`; refused by name beside `decision=publish`, and omitted stays valid. A recorded decision is not changed (an identical repeat is a no-op) |
| `urn:repo:{repo}:findings[:{path}]` | the **review queue** — every finding on one file or the whole repo, in TRIAGE order (severity, then position), `state=` one of `pending` (default) / `superseded` / `published` / `declined` / `all` — since 0.11.0 `pending` means undecided AND stood behind by the file's current review, and `superseded` is an undecided finding it does not carry (computed on read, never stored; see the review pass below; each row's `superseded_by` names the pass, else null); faces: `application/json` (default), `as=text/html` (the queue page, each pending card carrying its publish/decline form), `as=text/plain`, `as=text/turtle`. A row minted where a like claim was already **declined** on the same file carries that decision (`prior_decision`, see the review pass below); `summary=declined` widens the json face to `{repo, path, state, rows, declined: {count, reasons: [{reason, count}], quotes: [{exact, count, latest_decided_at, findings}]}}` — how many declines the file carries, by reason word (all five in contract order, zeros included, then `reason: null` for none stated) and by the quote they declined — and a file's html and plain listings say the same in words; `summary=states` widens it instead to `{repo, path, state, rows, states: {pending, superseded, published, declined, files: [{path, pending, superseded, superseded_by}]}}`, and a pending listing's html and plain faces say how many superseded findings they do not list. `group=` (0.12.0) proposes batches of PENDING findings for one human decision each — see *Finding groups* below. ⚠ A queue, not a gate: nothing here blocks a commit, a push or a merge |
| `urn:repo:{repo}:review:{path}` | the **machine review pass** (S4) — region-grain LLM commentary minted as **pending findings, never annotations** (each with the model's PROPOSED severity), the pass archived by `(path, content-hash, review-tag)` so re-sourcing unchanged content mints nothing — and, since 0.8.0, **every derived region memoized by the sha256 of its own bytes**, so a pass over a changed file asks the model only about the regions whose bytes moved and carries the unchanged regions' existing findings forward (same ids, same pending/declined state, re-anchored): the queue grows with the diff, not with the file. And since 0.9.0 **a decline is remembered**: a fresh finding whose exact quote matches a finding a human DECLINED on the same file is minted carrying that decision (`prov:wasInfluencedBy` the decision node; `prior_decision` on the row: the declined finding, its date, its reason word and its note), so declining it again is one click and a different claim on the same line stays visible; only an EXACT repeat — same quote, same proposed severity, byte-identical note — is withheld, and every withheld one is counted (`suppressed_items`, `ik:suppressedItems`, "(N withheld as exact repeats)" in the statement). It reports against a **threshold, not a quota**: every problem the model rates `critical` or `major`, however many or few, then at most three `minor`/`info` suggestions and at most one `praise` — a missed serious problem is silent and expensive, a rejected suggestion costs one click, so only the second half is bounded. ⚠ A file with nothing above the bar is an **archived pass with zero findings whose every face says so in words, with how much of the input was actually read** (`statement`); an empty answer that makes no such statement is still an error that archives nothing, because a quiet week and an outage must not look alike. Faces: `text/plain` (the margin digest), `as=application/json` (`{statement, minted, carried, memo_regions, derived_regions, orphaned_items, reviewed_bytes, total_bytes, annotations, …}` — `minted` is what THIS pass produced, `carried` what it carried forward, `annotations` the union), `as=text/html` (the card page), `as=text/turtle` (the pass's provenance graph); `debug=raw` derives and returns the model's **unparsed answer** (nothing minted or archived, and no memo consulted: every region derived by the rule alone) — the parse-failure diagnosis face; `provider={iri}` derives against a host-allowed backend, keyed by its own model identity (a second model is a second coexisting pass); `lens={name}` (0.13.0) reads the file through ONE lens instead of the general pass — `intent`, the constitution's four authority questions — the lens joining the archive key (`review-v5+intent@…`) so the lensed pass coexists with the plain one, the json face carrying `lens`, and every statement saying "through the intent lens" because a lensed all-clear is the narrower claim; the `one_of` is the lenses that exist, and `tests/corpus/intent/README.md` is the measurement it shipped on |
...
**The region memo (0.8.0, ledger #481).** A whole-file pass re-derived every
region at every new content hash, so the pending queue grew with FILE SIZE ×
COMMIT COUNT: one 108-line doc comment on a 181 KB file produced 57 findings,
nearly all on code the commit did not touch, and a file-hash throttle can never
fire on that because the hash did change. So the archive is keyed twice. Every
region a pass derives is memoized under `(repo, path, sha256 of the region's
bytes, tag)` — `urn:ikigai:browse:review-region:…`, `a prov:Collection` of the
finding records that call produced — and a fresh pass carries an unchanged
region's memo forward instead of asking: **the same finding records, same
ids, same pending/declined/published state**, re-anchored by the one drift path
(`prov:used <memo>` → `prov:hadMember <finding>` on the pass; `prov:generated`
names only what the pass minted itself). A declined finding on an unchanged
region stays declined; a pending one stays pending, once. "Substantial change"
is exact: the region's bytes moved. The tiling snaps to remembered regions —
without that, an insertion shifts every later cut point and the memo would miss
on everything below the edit — so an insertion or deletion costs the region it
lands in and nothing else. A region whose every quote misquoted gets no memo
(an empty memo would carry a clean claim the model never made), `debug=raw`
consults no memo, and a model or prompt swap re-derives every region because
the tag is in the key. Not this memo's job: a change in region 3 that breaks an
invariant stated in region 9 (the outline pass, ledger #451).

**A decline is remembered (0.9.0, ledger #475).** The memo covers an unchanged
region; what it cannot cover is a fresh derivation — a changed region, or every
region of a file's first pass after an upgrade re-keys the tag — and there a
declined claim returned under a new id, because the id carries the pass and the
pass carries the hash. Measured 2026-09-22: a prose-only commit to a manifest
minted 17 findings, 8 of them verbatim re-raises of the previous day's declines
on lines the commit did not touch. So at mint time, beside the place a misquote
is discarded and counted, a fresh finding is checked against the DECLINED
findings on the same file with the same `exact` (after the anchor's own
decoration strip — never position, hash or pass). By default it is **minted
marked**: it enters the queue carrying the declined twin's decision —
`prov:wasInfluencedBy <urn:iki:finding:{twin}:decision>` in the graph,
`prior_decision: {finding, iri, outcome, severity, decided_at, note, reason}` on the
json row, "a like claim on this line was declined (restates) 2026-09-21: <note>" on
the card (the one-word `reason` since 0.10.0) —
so a human declines it again in one click, or sees that it is a different claim,
which suppression could never show. Only an **exact repeat** — same quote, same
proposed severity, a byte-identical note — is **withheld**, and every withheld
one is counted (`suppressed_items` on the json face, `ik:suppressedItems` on the
pass, "(N withheld as exact repeats)" in the statement; a pass whose every item
was withheld is archived saying "nothing new"). A withheld item's memo member is
its declined twin, so unchanged bytes are not re-asked just to be withheld again.
The rule is deliberately narrow and was measured before it shipped: of the 458
declines on file, 98 pairs shared a line, 78 shared a severity, none shared a
byte-identical note. Declines on other files are not consulted — the same
misreading recurring across manifests or vocabularies is a knowledge gap in the
model, not something a key can carry — which is why the findings listing reports
the FILE grain instead (`summary=declined`, above).

**A re-review retires what it replaced (0.11.0, ledger #504).** The memo bounds
re-review of UNCHANGED regions; nothing retired the findings of a CHANGED one, so
the previous pass's findings there stayed pending forever and the queue grew with
commit count on a codebase that was not growing — 561 of 1697 pending findings on
2026-09-24 were in no standing set of their file's newest pass. So an undecided
finding is now **`superseded`** when its file's CURRENT reading does not stand
behind it: the reading is every pass over the file's current content hash (every
tag — a second `provider=` model is a second reviewer, not a replacement), else the
passes on the hash of the most recently derived pass; a pass stands behind what it
minted and what it carried through a memo. It is a pure function of the store, so
it is **computed on every read and stored nowhere**: no migration (the stock
retires the moment the build is read), nothing to drift, and a revert A → B → A
makes the pass over A current again and its findings pending again. Pending on a
file is then at most its current reading's standing set — bounded by codebase
size, not history. Superseded asserts no human judgment: it never feeds the
declined-twin mark, a human may still publish or decline it, decided findings
never change state, PR-page findings (keyed to a diff, not a file pass) are never
superseded, and a file with no pass on record supersedes nothing. The pass
statement says "N earlier findings superseded" (`superseded_items` on the json
face), and `summary=states` on the findings face counts it per root and per file.
Measured on a copy of the live graph (`examples/findings-read-probe.rs`), the
badge read over seven roots went from ~120 ms to ~106 ms: fewer rows to reconcile
and serialize, and the current content hash is the one the drift pass already
computes.

**Finding groups: the machine proposes, a human decides once (0.12.0, ledger
#506).** A decline records a HUMAN judgment and the recurrence mark reads every
decline as a prior "no", so nothing here auto-declines. Instead
`urn:repo:{repo}:findings[:{path}] group=<kind>` PROPOSES groups of pending
findings — superseded, published and declined never appear — each with a
suggested reason word read from the finding Sink's own `reason` one_of and never
applied. The kinds, in the contract's order: `recurrence` (pending findings
quoting a line where a like claim on the same file was already declined — the
0.9.0 mark computed backwards, over findings minted before it existed; one group
per declined twin, which rides along as `twin`, its word the suggestion),
`near-duplicate` (the same file, current line and proposed severity, reworded;
the oldest is `kept` and is not a member; suggests `duplicate`), `comment-shape`
(one file's findings whose quote is a comment or doc line — the `restates`
shape; ⚠ a mixed set to READ, sampled with real claims in it; suggests
`restates`) and `file` (everything pending on one file; no suggestion). The json
face is `{repo, path, state, group, kinds: [{kind, groups, findings}], groups:
[{kind, key, label, repo, path, annotates, reason, twin, kept, members}]}`, the
members in exactly the row shape the listing serves; html and plain faces say
the same. There is **no batch Sink**: a host (gonk's queue page) fans a batch out
to `urn:iki:finding:{id}`, one call per member, so each member gets an ordinary
decision node and a batch decline reads back — and marks — exactly like a single
one. Measured on a copy of the live graph (gonk's seven roots), a grouped read
costs what the pending read costs, and over 1185 pending the levers were
recurrence 78 groups / 79 findings, near-duplicate 6 / 6, comment-shape 36 / 255
and file 66 / 1185.

A finding whose quote does not anchor (the model
...
`prov:generated` / `prov:used` (and, since 0.9.0, `prov:wasInfluencedBy` from a
fresh finding to the decision node of a declined twin on the same line, and since
0.10.0 `dcterms:subject <urn:iki:decline-reason:{word}>` on a decision node — an
interim predicate for the decline reason until core defines `ik:declineReason`), plus
five `ik:` terms of its own: `ik:Review` (the pass-entry class),
`ik:orphanedItems` (the count of findings whose quotes did not anchor),
`ik:suppressedItems` (the count withheld as exact repeats of a decline, since
`ikigai-vocab` 0.1.71), `ik:reviewedBytes` and `ik:totalBytes` (how much of the
file the pass actually read, when the prompt ceiling truncated it). **All are in
